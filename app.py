...
required_models = [
    ...
    "models/vgg16_food_classification_model.keras",

    "models/food_classification_custom_Resnetmodel.keras",

    "models/food_classification_custom_model.h5"
]
...
def fetch_nutrition(class_name):
    ...
    return {
        ...
        "calories":
            str(entry.get("calories", "N/A")),

        "carbohydrates":
            str(entry.get("carbs", "N/A")),

        "fats":
            str(entry.get("fat", "N/A")),

        "proteins":
            str(entry.get("protein", "N/A")),

        "fiber":
            str(entry.get("fiber", "N/A"))
    }
...
def load_metrics(model_key, class_name):
    ...
    try:
        ...
        return {
            ...
            "precision":
                round(float(metrics.get("precision", 0)), 4),

            "recall":
                round(float(metrics.get("recall", 0)), 4),

            "f1_score":
                round(float(metrics.get("f1-score", 0)), 4),
